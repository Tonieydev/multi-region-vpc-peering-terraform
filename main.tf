...
resource "aws_instance" "primary_instance" {
  ...
  depends_on = [aws_vpc_peering_connection_accepter.secondary_accepter,
  aws_vpc_peering_connection_accepter.tertiary_accepter]
}
...
resource "aws_subnet" "tertiary_subnet_vpc" {
    provider = aws.tertiary
  region = var.tertiary
  vpc_id = aws_vpc.tertiary_vpc.id
  cidr_block = var.tertiaty_subnet_cidr
  availability_zone =  data.aws_availability_zones.tertiary.names[0]
   map_public_ip_on_launch = true

  tags = {
    Name = "tertiary-vpc-subnet-${var.tertiary}"
  }
}
...
resource "aws_route_table_association" "tertiary_rt_asssocation" {
    provider = aws.tertiary 
    region = var.tertiary
    route_table_id = aws_route_table.tertiary_rt.id
    subnet_id = aws_subnet.tertiary_subnet_vpc.id
} 
...
resource "aws_vpc_peering_connection" "primary_to_tertiary" {
  ...
  tags = {
    Name = "VPC Peering between ${var.primary} and ${var.tertiary}"
  }
}
...
resource "aws_route" "primary_tertiary_route" {
    provider = aws.primary
    region = var.primary
  route_table_id            = aws_route_table.primary_rt.id
...
resource "aws_route" "tertiary_primary_route" {
    provider = aws.tertiary
    region = var.tertiary
  route_table_id            = aws_route_table.tertiary_rt.id
  destination_cidr_block    = var.primary_vpc_cidr
